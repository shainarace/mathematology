...
######                   BASKETBALL DATA               #####
############################################################
setwd('/Users/shaina/Library/Mobile Documents/com~apple~CloudDocs/Social Network Analysis 2017/Potential Data Sources/CBasketball2013')
load("basketball2013.RData")
library(igraph)
basketball=as.undirected(basketball)
...
plot(c,basketball,edge.arrow.size=0.1, layout = l,vertex.label = V(basketball)$TeamName, vertex.size = 3, font.size=5, edge.color = "gray")

# Another method
c2=cluster_spinglass(basketball)
plot(c2,basketball,edge.arrow.size=0.1, layout = l, vertex.label = V(basketball)$TeamName, vertex.size = 3, font.size=7, edge.color = "gray")


# No one promised you nice clusters...
c3=cluster_label_prop(basketball)
plot(c3,basketball,edge.arrow.size=0.1, layout = l, vertex.label = V(basketball)$TeamName, vertex.size = 3, font.size=7, edge.color = "gray")

c4=cluster_fast_greedy(basketball)
plot(c4,basketball,edge.arrow.size=0.1, layout = l, vertex.label = V(basketball)$TeamName, vertex.size = 3, font.size=7, edge.color = "gray")
...
#' value of the modularity lies in the range (−1,1). 
...
# Interestingly enough the modularity maximization algorithm in R gives the minimum 
# modularity of the 4 clusterings on this particular graph...#MathIsPerfectAppliedMathIsImperfect
